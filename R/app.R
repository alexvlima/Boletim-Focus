...
source('../Documents/2021/029_Boletim_Focus/Aplicativo_Boletim_Focus/dataset.R')
...
credentials <- data.frame(
    user = c("alexandre.lima", "guilherme.coelho", "dirat", "babacar.sow"),
    password = c("100@Focus", "100@Focus", "100@Focus", "100@Focus"),
...
ui <- secure_app(head_auth = tags$script(inactivity),
                 ...
                 '),
                                    h1('Funcionamento'),
                                    h3('Para navegar, basta selecionar o tipo de indicador desejada na parte da esquerda. Depois, os dados são interativos, reagindo a sele\u00e7\u00e3o de m\u00e9tricas e período feitas. Ressalta-se que os dados aqui disponilizados s\u00e7\u00e3 desde janeiro/2020.
                  ')),
                            tabItem(tabName = 'pib',
                                    fluidRow(
                                        box(
                                    ...
                                    fluidRow(
                                        ...
                                        box(
                                            ...
                                            selectInput('metrica_pib', label = h4('M\u00e9trica:'), 
                                                        choices = list('Media' = 'mean', 'Mediana' = 'median', 'Minimo' = 'min', 'Maximo' = 'max', 'Desvio Padrao' = 'sd'),
                                                        selected = 'mean')
                                        )
                                    )
                            ),
...
server <- function(input, output) {
    ...
    base_pib1 <- reactive({
        pib %>%
            select(indic,date,reference_date, mean, median, min, max, sd) %>%
            gather("metric", "value", -c(indic,date,reference_date)) %>%
    ...
    output$Graf_PIB1 <- 
        renderPlotly({
            ...
            plot_ly(Base_pib1, 
                    x = ~reference_date, y=~value, mode = 'bar') %>%
    ...
    output$Graf_PIB2 <- 
        renderPlotly({
            Base_pib2 <- base_pib2() %>% dplyr::filter(reference_date==input$num_pib)
            plot_ly(Base_pib2, 
    ...
    output$Graf_Inflacao1 <- 
        renderPlotly({
            ...
            plot_ly(Base_inflacao1, 
                    x = ~reference_date, y=~value, type = 'bar') %>%
    ...
    base_inflacao2 <- reactive({
        inflacao %>%
            ...
            gather("metric", "value", -c(indic,date,reference_date)) %>%
            dplyr::filter(indic==input$indic_inflacao,
                          reference_date==input$num_inflacao,
                          metric == input$metrica_inflacao,
                          date >= input$data_inflacao[1] & date <= input$data_inflacao[2])
    })
    ...
    output$Graf_Inflacao2 <- 
        renderPlotly({
            Base_inflacao2 <- base_inflacao2() %>% dplyr::filter(reference_date==input$num_inflacao)
            plot_ly(
                Base_inflacao2, 
                x = ~date, y=~value, mode = 'lines') %>%
    ...
    base_cambio1 <- reactive({
        cambio %>%
            select(date,reference_date, mean, median, min, max, sd) %>%
            gather("metric", "value", -c(date,reference_date)) %>%
            dplyr::filter(metric == input$metrica_cambio,
    ...
    base_cambio2 <- reactive({
        cambio %>%
            select(date,reference_date, mean, median, min, max, sd) %>%
            gather("metric", "value", -c(date,reference_date)) %>%
            dplyr::filter(reference_date==input$num_cambio,
    ...
    output$Graf_Cambio2 <- 
        renderPlotly({
            Base_cambio2 <- base_cambio2() %>% dplyr::filter(reference_date==input$num_cambio)
            plot_ly(
                Base_cambio2, 
                x = ~date, y=~value, mode = 'lines') %>%
    ...
    base_selic1 <- reactive({
        selic %>%
            select(date,reference_date, mean, median, min, max, sd) %>%
            gather("metric", "value", -c(date,reference_date)) %>%
            dplyr::filter(metric == input$metrica_selic,
    ...
    base_selic2 <- reactive({
        selic %>%
            select(date,reference_date, mean, median, min, max, sd) %>%
            gather("metric", "value", -c(date,reference_date)) %>%
            dplyr::filter(reference_date==input$num_selic,
    ...
    output$Graf_Selic2 <- 
        renderPlotly({
            Base_selic2 <- base_selic2() %>% dplyr::filter(reference_date==input$num_selic)
            plot_ly(
                Base_selic2, 
                x = ~date, y=~value, mode = 'lines') %>%
    ...
    #     balanca_comercial %>%
    #         select(indic_detail,date,reference_date, mean, median, min, max, sd) %>%
    #         gather("metric", "value", -c(indic_detail,date,reference_date)) %>%
    ...
    #         gather("metric", "value", -c(indic_detail,date,reference_date)) %>%
    #         dplyr::filter(indic_detail==input$indic_balanca_comercial,
    #                       reference_date==input$num_balanca_comercial,
    #                       metric == input$metrica_balanca_comercial,
    #                       date >= input$data_balanca_comercial[1] & date <= input$data_balanca_comercial[2])
    # })
    # 
    ...
    #     renderPlotly({
    #         Base_balanca_comercial2 <- base_balanca_comercial2() %>% dplyr::filter(reference_date==input$num_balanca_comercial)
    #         plot_ly(
